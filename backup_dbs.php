...
$db_conn	= @mysqli_connect( $MYSQL_HOST, $MYSQL_USER, $MYSQL_PASSWD ) or error( true, mysql_error(), true );
...
while ($db_row = mysqli_fetch_object($db_result)) {
	...
	// compress db
	unset( $output );
	$password = (!empty($COMPRESSOR_PASS)) ? "-P '$COMPRESSOR_PASS'" : "";
	$nice     = ($os == 'unix') ? $USE_NICE : "";
	$command  = "$nice zip -mj $password $BACKUP_TEMP/$db.sql.zip $BACKUP_TEMP/$db.sql 2>&1";
	exec($command);
